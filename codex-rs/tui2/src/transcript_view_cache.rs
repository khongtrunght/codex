...
struct WrappedTranscriptCache {
    ...
    width: u16,
    /// Verbose mode this cache was last built for.
    verbose: bool,
...
impl WrappedTranscriptCache {
    ...
    /// - when `verbose` changes, rebuild from scratch (verbose affects cell rendering)
    ///
    ...
    fn ensure(&mut self, cells: &[Arc<dyn HistoryCell>], width: u16, verbose: bool) -> WrappedTranscriptUpdate {
        ...
        {
            self.rebuild(cells, width, verbose);
            return WrappedTranscriptUpdate::Rebuilt;
        }
    ...
    ///
    /// This is used when width changes, verbose changes, the transcript is truncated, or the
    /// caller provides a new cell list that cannot be treated as an append to the previous one.
    fn rebuild(&mut self, cells: &[Arc<dyn HistoryCell>], width: u16, verbose: bool) {
        self.width = width;
        self.verbose = verbose;
        self.cell_count = cells.len();
        self.first_cell_ptr = cells.first().map(Arc::as_ptr);
...
mod tests {
    ...
    fn wrapped_cache_ensure_appends_only_new_cells_when_width_is_unchanged() {
        ...
        let mut cache = TranscriptViewCache::new();
        cache.ensure_wrapped(&cells[..1], 8, false);
        cache.ensure_wrapped(&cells, 8, false);
    ...
    fn wrapped_cache_ensure_rebuilds_on_width_change() {
        ...
        let mut cache = TranscriptViewCache::new();
        cache.ensure_wrapped(&cells, 8, false);
        cache.ensure_wrapped(&cells, 10, false);

        assert_eq!(calls0.load(Ordering::Relaxed), 2);
        assert_eq!(calls1.load(Ordering::Relaxed), 2);

        let expected = crate::transcript_render::build_wrapped_transcript_lines(&cells, 10, false);
        assert_eq!(cache.lines(), expected.lines.as_slice());
        assert_eq!(cache.line_meta(), expected.meta.as_slice());
        assert_eq!(
            cache.wrapped.transcript.joiner_before,
    ...
    fn wrapped_cache_ensure_with_zero_width_clears_without_calling_cell_render() {
        ...
        let mut cache = TranscriptViewCache::new();
        cache.ensure_wrapped(&cells, 0, false);
    ...
    fn wrapped_cache_ensure_rebuilds_when_first_cell_pointer_changes() {
        ...
        let mut cache = TranscriptViewCache::new();
        cache.ensure_wrapped(&[cell_a0.clone(), cell_a1.clone()], 10, false);
        ...
        cache.ensure_wrapped(&[cell_b0.clone(), cell_a1.clone()], 10, false);
    ...
    #[test]
    fn wrapped_cache_ensure_rebuilds_on_verbose_change() {
        let calls0 = Arc::new(AtomicUsize::new(0));
        let calls1 = Arc::new(AtomicUsize::new(0));
        let cells: Vec<Arc<dyn HistoryCell>> = vec![
            Arc::new(FakeCell::new(
        ...
        // Initial build should render both cells once.
        assert_eq!(calls0.load(Ordering::Relaxed), 1);
        assert_eq!(calls1.load(Ordering::Relaxed), 1);

        // Toggling verbose should trigger a full rebuild.
        cache.ensure_wrapped(&cells, 8, true);
        assert_eq!(calls0.load(Ordering::Relaxed), 2);
        assert_eq!(calls1.load(Ordering::Relaxed), 2);

        // Toggling back should also rebuild.
        cache.ensure_wrapped(&cells, 8, false);
        assert_eq!(calls0.load(Ordering::Relaxed), 3);
        assert_eq!(calls1.load(Ordering::Relaxed), 3);

        // Same verbose value should not rebuild.
        cache.ensure_wrapped(&cells, 8, false);
        assert_eq!(calls0.load(Ordering::Relaxed), 3);
        assert_eq!(calls1.load(Ordering::Relaxed), 3);
    }
    ...
    fn raster_cache_reuses_rows_and_clears_on_width_change() {
        ...
        cache.ensure_wrapped(&cells, 20, false);
