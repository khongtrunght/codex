...
use crate::unified_exec::ProcessTerminateResult;
...
impl UnifiedExecProcessManager {
    ...
    /// Get output from a process by ID (read-and-drain pattern).
    /// Returns accumulated output since last drain.
    pub async fn get_process_output(
        &self,
        process_id: &str,
    ) -> Result<ProcessOutputSnapshot, UnifiedExecError> {
        ...
        let OutputHandles { output_buffer, .. } = entry.process.output_handles();
    ...
    pub async fn terminate_process(
        &self,
        process_id: &str,
    ) -> Result<ProcessTerminateResult, UnifiedExecError> {
        let mut store = self.process_store.lock().await;
        let entry = store
            .remove(process_id)
            .ok_or(UnifiedExecError::UnknownProcessId {
        ...
        let was_running = !entry.process.has_exited();
        entry.process.terminate();
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_process_output_unknown_process() {
        let manager = UnifiedExecProcessManager::default();
        let result = manager.get_process_output("unknown_id").await;
        assert!(matches!(
            result,
            Err(super::super::UnifiedExecError::UnknownProcessId { .. })
        ));
    }

    #[tokio::test]
    async fn test_terminate_process_unknown() {
        let manager = UnifiedExecProcessManager::default();
        let result = manager.terminate_process("unknown_id").await;
        assert!(matches!(
            result,
            Err(super::super::UnifiedExecError::UnknownProcessId { .. })
        ));
