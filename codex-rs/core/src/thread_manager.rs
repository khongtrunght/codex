...
/// [`ThreadManager`] is responsible for creating threads and
...
impl ThreadManager {
    ...
    pub async fn get_thread(&self, conversation_id: ThreadId) -> CodexResult<Arc<CodexThread>> {
        self.state.get_thread(conversation_id).await
    }
...
impl ThreadManagerState {
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> CodexResult<Arc<CodexThread>> {
        let conversations = self.threads.read().await;
        conversations
            .get(&thread_id)
            .cloned()
