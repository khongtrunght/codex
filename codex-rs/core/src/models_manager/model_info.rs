...
macro_rules! model_info {
    (
        ...
        // Render GeneralMainPrompt if base_instructions wasn't explicitly set (None or empty)
        if model.base_instructions.as_ref().map_or(true, |s| s.is_empty()) {
            model.base_instructions = Some(render_general_prompt(model.edit_tool_type, model.shell_type));
        }
...
pub(crate) fn find_model_info_for_slug(slug: &str) -> ModelInfo {
        ...
        && !slug.contains("codex")
    {
        ...
        model_info!(
            slug,
            edit_tool_type: edit_tool_type,
            shell_type: shell_type,
            base_instructions: Some(render_general_prompt(edit_tool_type, shell_type)),
            supports_parallel_tool_calls: true,
            context_window: None,
