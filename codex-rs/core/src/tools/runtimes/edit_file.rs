...
//! for consistent approval and execution flow. Implements multi-strategy
//! matching: exact → normalized whitespace → line-trimmed.
...
impl Approvable<EditFileRequest> for EditFileRuntime {
    ...
    fn start_approval_async<'a>(
        ...
        Box::pin(async move {
            ...
            let preview_content = content.replacen(&old_string, &new_string, 1);
            let unified_diff = diffy::create_patch(&content, &preview_content).to_string();
            ...
            with_cached_approval(&session.services, "edit_file", approval_keys, || async {
                ...
                patch_changes.insert(
                    ...
                    ApplyPatchFileChange::Update {
                        unified_diff: unified_diff.clone(),
                        move_path: None,
                        new_content: preview_content.clone(),
                    },
                );
...
fn try_replace(
    ...
    if normalized_content.contains(&normalized_old)
        && let Some(result) = replace_with_normalized(content, old_string, new_string, replace_all)
    {
