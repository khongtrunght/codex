...
impl AskUserQuestionOverlay {
    ...
    fn build_tab_bar(&self, _width: u16) -> Line<'static> {
        ...
        for (idx, q) in self.questions.iter().enumerate() {
            ...
            if is_current {
                // Light background with bold text for focused tab
                spans.push(tab_text.on_light_blue().bold());
            } else {
                spans.push(tab_text.into());
            }
        }
        ...
        if self.questions.len() > 1 {
            ...
            let tab_text = format!(" {TICK} Submit ");
    ...
    fn build_question_options(&self) -> Vec<Line<'static>> {
        ...
        for (idx, opt) in q.options.iter().enumerate() {
            ...
            if q.multi_select {
                ...
                spans.push(format!("{prefix} {num}. ").into());

                // Checkbox: green if checked
                if is_checked {
                    spans.push(check_mark.green());
                    spans.push(format!(" {label}", label = opt.label).green());
                } else if is_focused {
                    spans.push(check_mark.into());
                    spans.push(format!(" {label}", label = opt.label).light_blue());
                } else {
                    spans.push(format!("{check_mark} {label}", label = opt.label).into());
                }
                ...
                let label_line = format!("{prefix} {num}. {label} {TICK_GREEN}", label = opt.label);
                lines.push(Line::from(label_line).green());
            } else if is_focused {
                // Single-select focused: blue color
                let label_line = format!("{prefix} {num}. {label}", label = opt.label);
            ...
            if !opt.description.is_empty() {
                let desc_line =
                    Line::from(format!("   {description}", description = opt.description)).dim();
                if is_checked {
        ...
        if is_typing_custom {
            ...
            if q.multi_select {
                ...
                if is_other_checked {
                    spans.push(check_mark.green());
                    spans.push(format!(" {other_label}").green());
                } else if is_other_selected {
                    spans.push(check_mark.into());
                    spans.push(format!(" {other_label}").light_blue());
                } else {
                    spans.push(format!("{check_mark} {other_label}").into());
                }
    ...
    fn build_submit_screen(&self) -> Vec<Line<'static>> {
        ...
        if !self.all_questions_answered() {
            lines.push(Line::from(vec![
                WARNING.yellow(),
                " You have not answered all questions".yellow(),
            ]));
            lines.push(Line::from(""));
        }
        ...
        let submit_line = format!("{submit_prefix} 1. Submit answers");
        let cancel_line = format!("{cancel_prefix} 2. Cancel");
