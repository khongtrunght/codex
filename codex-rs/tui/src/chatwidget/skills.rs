...
use codex_core::config::types::SkillsConfig;
...
impl ChatWidget {
    ...
    pub(crate) fn open_manage_skills_popup(&mut self) {
        ...
        for skill in &self.skills_all {
            initial_state.insert(
                normalize_skill_config_path(&skill.metadata.path),
                skill.enabled,
            );
        }
        ...
        let items: Vec<SkillsToggleItem> = self
            ...
            .iter()
            .map(|skill| {
                let display_name = skill_display_name(&skill.metadata).to_string();
                let description = skill_description(&skill.metadata).to_string();
                let name = skill.metadata.name.clone();
                let path = skill.metadata.path.clone();
                SkillsToggleItem {
                    name: display_name,
                    skill_name: name,
                    description,
    ...
    pub(crate) fn update_skill_enabled(&mut self, path: PathBuf, enabled: bool) {
        ...
        for skill in &mut self.skills_all {
            if normalize_skill_config_path(&skill.metadata.path) == target {
    ...
    pub(crate) fn set_skills_from_response(&mut self, response: &ListSkillsResponseEvent) {
        ...
        if let Some(user_layer) = self.config.config_layer_stack.get_user_layer()
            && let Some(skills_value) = user_layer.config.get("skills")
        {
            let skills_config: SkillsConfig = match skills_value.clone().try_into() {
                Ok(skills) => skills,
                Err(err) => {
            ...
            for entry in skills_config.config {
                config_overrides.insert(
                    normalize_skill_config_path(entry.path.as_path()),
                    entry.enabled,
                );
            }
        }
        ...
        let loaded = load_skills(&self.config);
        let skills = loaded
            .skills
            .into_iter()
            .map(|skill| {
                let normalized = normalize_skill_config_path(&skill.path);
                let enabled = existing_overrides
                    .get(&normalized)
                    .or_else(|| config_overrides.get(&normalized))
                    .copied()
                    .unwrap_or(true);
                SkillState {
                    metadata: skill,
...
fn enabled_skills_for_mentions(skills: &[SkillState]) -> Vec<SkillMetadata> {
    skills
        .iter()
